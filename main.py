...
bot = commands.Bot(command_prefix="ㄱ")
...
async def 도움(ctx):
    ...
    embed.add_field(name = bot.command_prefix + "홀짝 홀 [돈]", value = "[돈]을 걸어 홀짝게임을 합니다. 올인도 가능합니다", inline = False)
            ...
            if cur_money >= int(money):
                if face == "홀" or face == "짝":
                    if forecast == face:
                        ...
                        modifyMoney(ctx.author.name, userRow, 2 * betting)
                    else:
                        result = "실패"
                        _color = 0xFF0000
